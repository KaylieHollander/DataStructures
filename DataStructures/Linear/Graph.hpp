...
//  DataStructures
//
//  Created by Hollander, Kaylie on 3/6/19.
...
/*
 Since we cannot "remove" from an array in C++, we only implement adding to a Graph. Java allows for all objeccts to be set to null but C++ does not.
 */
...
void Graph<Type> :: depthFirstTraversal(Graph<Type> & currentGraph, int vertex, bool * visited)
{
    ...
    for(setIterator = connections.begin(); setIterator != connections.end(); setIterator++)
    {
        ...
        {
            depthFirstTraveral(currentGraph, *setIterator, visited);
        }
    }
}
...
int Graph<Type> :: costTraversal(Graph<Type> & currentGraph, int start)
{
    ...
    std::set<int> connections;
    std::set<int>::iterator setIterator;
    std::queue<int> vetexQueue;
    
    std::fill_n(visited, currentGraph.size(), false);
    visited[start] = true;
    
    vetexQueue.push(start);
    while (!vetexQueue.empty())
    {
        int currentIndex = vetexQueue.front();
